"""
Per-cell correlation: head_pageview vs min_pageview, against two
targets — Direct EM and Thought-Gap sign.

For each (model, task) cell:
  Inputs:
    - pageview_min_<task>.jsonl  → head_pageview, min_pageview per qid
    - probe_direct_<model>_<task>.jsonl  → Direct EM per qid
    - pilot_full_<model>_<task>_seed{42,7,123}.jsonl  → EM_full per qid
    - pilot_none_<model>_<task>_seed{42,7,123}.jsonl  → EM_none per qid
    Δ(q) = mean_seeds(EM_full(q)) - mean_seeds(EM_none(q))
    sign(Δ) ∈ {-1, 0, +1}

  Per (signal_type, target) we report:
    - Pearson r + 95 % bootstrap CI (2000 reps, by qid) + two-sided p
    - Spearman ρ + 95 % bootstrap CI + p
    - For target = thought_gap_sign: also ordered-logit coefficient + p
...
OUT_CSV = SIG_DIR / "correlation_min_vs_head.csv"
...
def _pilot_path(variant: str, model_slug: str, task: str,
                n: int, seed: int) -> Path:
    """Build pilot log path; handle V3-hotpotqa legacy naming."""
...
def load_pilot_em(variant: str, model_slug: str, task: str,
    ...
    """qid → [EM, EM, …] across the seeds we find data for. Tries n=200 first."""
...
def thought_gap_sign(qid: str,
    ...
    """sign(mean_seeds(EM_full(q)) - mean_seeds(EM_none(q)))"""
    ef = em_full.get(qid)
    en = em_none.get(qid)
    if not ef or not en:
        return None
...
def _ordered_logit(signal: np.ndarray, target: np.ndarray):
    """Fit OrderedModel(target ∈ {-1,0,+1}) ~ signal. Return (coef, p)."""
    try:
        from statsmodels.miscmodels.ordinal_model import OrderedModel
        if len(np.unique(target)) < 2:
            return None
        # Treat target as ordinal
...
SIGNAL_FIELDS = {"head": "head_pv", "min": "min_pv"}
...
def main() -> None:
    ...
    for task in TASKS:
        ...
        if not sig:
            print(f"  [{task}] no pageview_min file; skip")
            continue
...
def print_table(rows: list[dict]) -> None:
    print("=" * 110)
    print("  per-cell correlation: head_pageview / min_pageview vs target")
    print("=" * 110)
